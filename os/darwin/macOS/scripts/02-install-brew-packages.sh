...
HEAD_FORMULAS=$(brew list --versions | grep 'HEAD-' | awk '{print $1}')
if [ -n "${HEAD_FORMULAS}" ]; then
    echo "${HEAD_FORMULAS}" | xargs brew upgrade --fetch-HEAD 2>&1 | grep -v "already installed" | grep -v "^Warning:" || true
fi
...
opencode completion zsh >/opt/homebrew/share/zsh-completions/_opencode
